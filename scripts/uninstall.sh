...
clean_tools() {
    ...
    # ~/.local/bin 下本项目装的二进制
    for b in zoxide starship eza yazi uv uvx rich nvitop gpustat; do
    ...
    if [ -n "$recorded" ] && [ "$recorded" != "$HOME/.local" ]; then
        ...
        for b in zoxide starship eza yazi uv uvx myenv rich; do
        ...
        rm_path "prefix/share/uv" "$recorded/share/uv"
    fi
